...
const Buypage = () => {
    const {isLoggedIn,handleLogin,updateUser,setUserId,user} = useContext(UserContext); // ดึงค่ามาจาก useContext ในไฟล์ user 
    const {cart} = useContext(CartContext); // ดึงค่ามาจาก useContext ในไฟล์ useContext
    ...
    const [product,setProduct] = useState<Product>([]); // สร้าง useState เพื่อเก็บข้อมูลสินค้าจากการ Fetch
    ...
    const result = Math.round((product.price - ((product.price * product.discountPercentage)/100))*100)/100 // เป็นตัวแปรสำหรับแสดงราคาสินค้าหลังจาก Discount
  return (
    <div>
